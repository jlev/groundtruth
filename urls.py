...
urlpatterns += patterns('groundtruth.geo.views',
    ...
    (r'(?P<model_name>[\w-]+).shp$','shapefile_view'),
    ...
    (r'region/$','region_list'),
    (r'region/(?P<id>\d+)/$','region_page'),
    (r'palestinian/(?P<id>\d+)/$','palestinian_page'),
    (r'palestinian/(?P<id>\d+)/popup/$','palestinian_popup'),
